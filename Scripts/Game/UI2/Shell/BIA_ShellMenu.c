...
class BIA_ArsenalTabViewComponent : SCR_TabViewComponent
{
	protected BIA_ShellMenu m_TransitionGuard;

	void SetTransitionGuard(BIA_ShellMenu guard)
	{
...
//! and all state lives in BIA_DraftService.
class BIA_ShellMenu : SCR_SuperMenuBase
{
	protected static bool s_bOpen;
	protected static BIA_ShellMenu s_ActiveInstance;
	...
	protected BIA_ArsenalTabViewComponent m_TabView;
	...
	override void OnMenuOpen()
	{
		...
		BIA_KitStore.TouchConsoleModule();
		BIA_KitStore.FlushIfPending();

		BIA_DraftService service = BIA_DraftService.Get();
		if (!service)
		{
			BIA_Log.Warn("Shell opened without a draft session, closing");
			Close();
			return;
		}
		...
		if (m_SuperMenuComponent)
			m_TabView = BIA_ArsenalTabViewComponent.Cast(m_SuperMenuComponent.GetTabView());
		if (m_TabView)
			m_TabView.SetTransitionGuard(this);
		else
			BIA_Log.Error("Shell: guarded Arsenal tab view is missing");
		...
		if (!m_wLeavePrompt || !m_LeaveYesButton || !m_LeaveNoButton)
			BIA_Log.Error("Shell: Gunsmith leave prompt is incomplete");
		if (m_LeaveYesButton)
		{
	...
	override void OnMenuUpdate(float tDelta)
	{
		...
		GetGame().GetInputManager().ActivateContext("BIA_ArmoryContext");
	...
	override void OnMenuClose()
	{
		...
		if (service && service.m_bDraftDirty && service.m_Config && service.m_Config.m_bApplyOnClose
			&& BIA_ClientPrefs.Get().m_bApplyOnClose)
		...
		BIA_StageHub.Shutdown();
		m_Hub = null;
		BIA_DraftService.Clear();
	...
	static bool OpenGunsmithForClothing(int clothingSlot)
	{
		...
		BIA_GunsmithScreen.QueueClothingInspection(clothingSlot);
	...
	static void ShowStatus(string text, bool success)
	{
		...
		Color color = BIA_Theme.TextPrimary();
		if (!success)
			color = BIA_Theme.Separator();
	...
	protected void OnBack()
	{
		...
		if (BIA_SoldierScreen.ConsumeBack())
			return;

		if (BIA_GunsmithScreen.ConsumeBack())
			return;

		if (BIA_KitsScreen.ConsumeBack())
			return;
	...
	bool RequestTabChange(int targetTab, bool callAction, bool playSound)
	{
		if (m_iPendingTab >= 0)
			return false;
		if (!BIA_GunsmithScreen.HasActiveAttachmentEdit())
			return true;
		if (!m_wLeavePrompt || !m_LeaveYesButton || !m_LeaveNoButton)
		{
			BIA_Log.Error("Shell: leave prompt unavailable; preserving the attachment draft and switching tabs");
			BIA_GunsmithScreen.SaveAndCloseAttachmentEdit();
			return true;
		}
	...
	protected void CancelPendingTabChange()
	{
		...
		if (m_TabView)
			m_TabView.ReassertCurrentButton();
		BIA_GunsmithScreen.RestoreAttachmentEditFocus();
	}
	...
	protected void OnApplyResult(BIA_EApplyStatus status, int applied, int skipped, float suppliesCharged, string skippedSample)
	{
		...
		string text;
		Color statusColor = BIA_Theme.TextPrimary();
		switch (status)
		{
			case BIA_EApplyStatus.SUCCESS:
			{
				text = "WORN";
				break;
			}
			case BIA_EApplyStatus.PARTIAL:
			{
				text = string.Format("WORN: %1 ITEMS UNAVAILABLE", skipped);
				statusColor = BIA_Theme.Separator();
				break;
			}
			case BIA_EApplyStatus.FAILED_RANK:
			{
				text = "HIGHER RANK REQUIRED";
				statusColor = BIA_Theme.Separator();
				break;
			}
			case BIA_EApplyStatus.FAILED_SUPPLIES:
			{
