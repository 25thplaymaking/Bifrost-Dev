...
class BIA_SoldierStage
{
	...
	//------------------------------------------------------------------------------------------------
	void BIA_SoldierStage(notnull BIA_StageCore core)
	{
		m_Core = core;
	...
	//------------------------------------------------------------------------------------------------
	void RefreshFromDraft(notnull BIA_DraftService service)
	{
		if (service.m_Draft)
	...
	void RefreshFromKit(notnull BIA_Kit kit, notnull BIA_DraftService service)
	{
	...
	protected bool EnsureCharacterSource(ResourceName characterPrefab)
	{
		...
		{
			BIA_Log.Warn("Soldier stage: pooled character failed to resolve");
			return false;
		}
		...
		{
			BIA_Log.Warn("Soldier stage: character has no preview-capable inventory storage");
	...
	protected bool RecloneCharacter()
	{
		...
		m_Character = m_CharacterStorage.CreatePreviewEntity(m_Core.GetWorld(), BIA_StageCore.CAMERA);
		if (!m_Character)
		{
			BIA_Log.Warn("Soldier stage: animated preview clone failed");
			return false;
		}
		...
		{
			BIA_Log.Warn("Soldier stage: preview clone has no animation component");
			DeleteRenderedCharacter();
			return false;
		}
