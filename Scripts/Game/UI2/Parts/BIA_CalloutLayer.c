...
class BIA_CalloutLayer
{
	...
	protected BIA_WeaponStage m_Stage;
	protected ref array<ref BIA_CalloutEntry> m_aEntries = {};
	...
	//------------------------------------------------------------------------------------------------
	void BIA_CalloutLayer(Widget calloutRoot, BIA_WeaponStage stage)
	{
		m_wRoot = calloutRoot;
		m_Stage = stage;
	}
	...
	//------------------------------------------------------------------------------------------------
	BIA_CalloutEntry GetEntry(int index)
	{
		if (index < 0 || index >= m_aEntries.Count())
			return null;
	...
	int FindEntryByAttachment(ResourceName attachedPrefab, int storageSlot = -1)
	{
		if (attachedPrefab.IsEmpty())
			return -1;

		foreach (int i, BIA_CalloutEntry entry : m_aEntries)
		{
			if (entry.m_AttachedPrefab == attachedPrefab && (storageSlot < 0 || entry.m_iStorageSlot == storageSlot))
				return i;
		}
		return -1;
	}
	...
	void SetChipsVisible(bool visible)
	{
		...
		foreach (BIA_CalloutEntry entry : m_aEntries)
		{
			if (entry.m_wChip)
	...
	void Build(notnull IEntity weaponEntity, SCR_Faction faction)
	{
		...
		BaseInventoryStorageComponent storage = BIA_ItemIntel.AttachmentStorage(weaponEntity);
		if (!storage)
			return;
		...
		for (int i = 0; i < count; ++i)
		{
			...
			if (entry.m_wChip)
			{
				...
				BIA_ItemRowComponent row = BIA_ItemRowComponent.Cast(entry.m_wChip.FindHandler(BIA_ItemRowComponent));
				if (row)
				{
					string state = "EMPTY";
					if (!entry.m_AttachedPrefab.IsEmpty())
						state = BIA_CatalogService.GetDisplayName(entry.m_AttachedPrefab, faction);
	...
	//------------------------------------------------------------------------------------------------
	protected void OnChipClicked(BIA_ItemRowComponent row)
	{
		foreach (int i, BIA_CalloutEntry entry : m_aEntries)
		{
			if (entry.m_Row == row)
		...
		foreach (int i, BIA_CalloutEntry entry : m_aEntries)
		{
			...
			if (entry.m_wDot)
			{
				...
				if (i == m_iHighlight)
				{
					...
					entry.m_wDot.SetColor(BIA_Theme.Separator());
				}
				else
				{
					FrameSlot.SetSize(entry.m_wDot, DOT_SIZE, DOT_SIZE);
	...
	protected bool TryPlaceColumns()
	{
		...
		array<ref BIA_CalloutEntry> column = {};
		foreach (BIA_CalloutEntry entry : m_aEntries)
		{
			vector anchor;
	...
	protected void SortByAnchor(notnull array<ref BIA_CalloutEntry> column)
	{
		for (int i = 1; i < column.Count(); ++i)
		{
			ref BIA_CalloutEntry current = column[i];
			int j = i - 1;
	...
	//------------------------------------------------------------------------------------------------
	protected void StackColumn(notnull array<ref BIA_CalloutEntry> column, float chipX)
	{
		float nextY = COLUMN_TOP;
		foreach (BIA_CalloutEntry entry : column)
		{
	...
	protected string SlotLabel(notnull InventoryStorageSlot slot, inout int anonymousSlotIndex)
	{
		...
		string mountLabel = BIA_ItemIntel.MountLabel(slot);
		if (!mountLabel.IsEmpty()) return mountLabel;
		return NextAttachmentLabel(anonymousSlotIndex);
	}
