class BIA_ReceiverCardHandler : ScriptedWidgetEventHandler
{
	protected BIA_ReceiverBrowser m_Owner;

	void BIA_ReceiverCardHandler(BIA_ReceiverBrowser owner)
	{
		m_Owner = owner;
	}
...
class BIA_ReceiverBrowser
{
	...
	protected BIA_ItemRowComponent m_Row;
	protected BIA_TileStrip m_Strip;
	protected ref array<BIA_ArmoryCategory> m_aCategories = {};
	protected ref array<ref BIA_ItemEntry> m_aItems;
	protected int m_iClass = -1;
	protected ResourceName m_StagedPrefab;
	protected int m_iStagedSlot = -1;
	protected ref BIA_ReceiverCardHandler m_CardHandler;
	...
	//------------------------------------------------------------------------------------------------
	void BIA_ReceiverBrowser(Widget screenRoot, BIA_TileStrip strip)
	{
		m_Strip = strip;
		if (!screenRoot)
			return;
		...
		if (cardButton)
		{
			m_CardHandler = new BIA_ReceiverCardHandler(this);
		...
		m_Row = BIA_ItemRowComponent.Cast(rowRoot.FindHandler(BIA_ItemRowComponent));
		if (m_Row)
	...
	BIA_ItemEntry GetStagedEntry()
	{
		...
		return BIA_CatalogService.FindEntry(m_StagedPrefab, m_aCategories, service.GetBrowseFaction(), service.m_Arsenal, service.GetCostType());
	}
	...
	void OpenBrowser()
	{
		...
		int initial = 0;
		foreach (int i, BIA_ArmoryCategory category : m_aCategories)
		{
		...
		array<string> labels = {};
		foreach (BIA_ArmoryCategory chipCategory : m_aCategories)
	...
	void ShowClass(int index)
	{
		...
		BIA_ArmoryCategory category = m_aCategories[index];
		m_aItems = BIA_CatalogService.GetItems(category, service.GetBrowseFaction(), service.m_Arsenal, service.GetCostType());

		array<BIA_ItemEntry> display = {};
		if (m_aItems)
		{
			foreach (BIA_ItemEntry item : m_aItems)
				display.Insert(item);
		}
	...
	bool HandleTileClicked(BIA_ItemRowComponent row)
	{
		if (!IsBrowsing())
			return false;
	...
	//------------------------------------------------------------------------------------------------
	protected void EnsureCategories(notnull BIA_DraftService service)
	{
		if (m_aCategories.IsEmpty())
			BIA_CatalogService.GetWeaponSlotCategories(service.m_Config, m_aCategories);
	}
	...
	protected void RefreshCard()
	{
		...
		BIA_ItemEntry entry = GetStagedEntry();
		if (entry)
			m_Row.SetEntry(entry, service.UsesSupplies());
		else
			m_Row.SetSlotDisplay(BIA_CatalogService.GetDisplayName(m_StagedPrefab, service.GetBrowseFaction()), "", m_StagedPrefab);
