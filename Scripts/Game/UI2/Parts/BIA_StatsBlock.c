...
class BIA_StatsBlock
{
	...
	//------------------------------------------------------------------------------------------------
	void BIA_StatsBlock(Widget screenRoot)
	{
		if (!screenRoot)
			return;
		...
		m_sLabelColor = UIColors.FormatColor(BIA_Theme.Separator());
	}
	...
	void Refresh(IEntity weaponSource, ResourceName weaponPrefab, BIA_ItemEntry entry, string displayName, IEntity character)
	{
		if (!m_wRoot || !m_wText)
			return;
		...
		if (entry && BIA_DraftService.RanksActive() && entry.m_eRequiredRank > SCR_ECharacterRank.PRIVATE)
