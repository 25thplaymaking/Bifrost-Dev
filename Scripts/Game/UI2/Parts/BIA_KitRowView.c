...
class BIA_KitRowView
{
	...
	//! (BIA_KitRowView row, string newName) rename left the edit box with a changed, non-empty name.
	ref ScriptInvoker m_OnRenameCommitted = new ScriptInvoker();

	//------------------------------------------------------------------------------------------------
	static BIA_KitRowView Create(Widget listParent, int slot, notnull BIA_KitFile kit)
	{
		if (!listParent)
			return null;
		...
		BIA_KitRowView row = new BIA_KitRowView();
		row.m_iSlot = slot;
		row.m_Kit = kit;
		row.m_wRoot = w;
	...
	void FillMeta(BIA_DraftService service)
	{
		if (!m_Kit)
			return;
		...
		if (m_Kit.m_iRequiredRank > 0 && BIA_DraftService.RanksActive())
	...
	void ShowResultChip(string label, bool failed)
	{
		...
		if (chipText)
		{
			...
			if (failed)
				...
				chipText.SetColor(BIA_Theme.Accent());
		}
		...
		if (chipBorder)
		{
			if (failed)
				...
				chipBorder.SetColor(BIA_Theme.AccentDeep());
		}
