...
class BIA_TileStrip
{
	...
	protected BIA_CarouselComponent m_Carousel;
	...
	//! (BIA_ItemRowComponent row) tile activated.
	...
	//------------------------------------------------------------------------------------------------
	void BIA_TileStrip(Widget screenRoot)
	{
		if (!screenRoot)
			return;
		...
		if (m_wCarouselViewport)
			m_Carousel = BIA_CarouselComponent.Cast(m_wCarouselViewport.FindHandler(BIA_CarouselComponent));
	...
	void ShowTiles(string title, notnull array<BIA_ItemEntry> items, ResourceName markedPrefab, string markedText, bool usesSupplies)
	{
		if (!m_wPanel || !m_wList)
			return;
		...
		foreach (BIA_ItemEntry item : items)
		{
			...
			BIA_ItemRowComponent row = BIA_ItemRowComponent.Cast(tileRoot.FindHandler(BIA_ItemRowComponent));
			if (!row)
			{
	...
	float GetVisibleHeight()
	{
		...
		if (m_wClasses && m_wClasses.IsVisible()) height += PANEL_HEIGHT_WITH_CLASSES - PANEL_HEIGHT;
		return height;
	}
	...
	void RefreshPanelHeight()
	{
		...
		if (panel) panel.SetHeightOverride(GetVisibleHeight());
	}
	...
	//------------------------------------------------------------------------------------------------
	protected void RevealRow(BIA_ItemRowComponent row)
	{
		if (!m_Carousel || !row)
			return;
