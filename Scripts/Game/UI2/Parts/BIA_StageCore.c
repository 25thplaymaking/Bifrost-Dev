...
class BIA_StageDragHandler : ScriptedWidgetEventHandler
{
	...
	//------------------------------------------------------------------------------------------------
	void BIA_StageDragHandler(notnull RenderTargetWidget target, notnull BIA_StageCore core)
	{
		m_Target = target;
		m_Core = core;
...
class BIA_StageCore
{
	...
	protected ref BIA_StageDragHandler m_DragHandler;
	...
	bool EnsureWorld(string namePrefix)
	{
		...
		if (!m_WorldRef)
		{
			BIA_Log.Error("StageCore: CreateWorld failed");
			return false;
		}
		...
		if (!m_World)
		{
			BIA_Log.Error("StageCore: world reference is empty");
			m_WorldRef = null;
			return false;
		}
		...
		if (m_EnvironmentRig)
		{
			IEntity stand = m_World.FindEntityByName("BIA_ArmorStand");
			if (stand) stand.ClearFlags(EntityFlags.VISIBLE, true);
			BindStudioLights();
			if (m_bStudioLightingAllowed && (!m_StudioKey || !m_StudioFillLeft || !m_StudioFillRight))
				BIA_Log.Warn("StageCore: camera-relative studio lights are missing from the environment rig");
		}
		else
			BIA_Log.Warn("StageCore: environment failed to load, stage will be dark");
	...
	void BindTarget(notnull RenderTargetWidget target)
	{
		...
		if (m_DragHandler)
			...
			m_DragHandler = new BIA_StageDragHandler(target, this);
	...
	//------------------------------------------------------------------------------------------------
	// Keep direct references; world-environment entities need not be transform children.
	protected void BindStudioLights()
	{
		m_StudioKey = null;
		m_StudioFillLeft = null;
		m_StudioFillRight = null;
		if (!m_bStudioLightingAllowed || !m_World) return;
		Resource lightPrefab = Resource.Load("{AB205AD4E2000011}Prefabs/UI/GRSA_StageLight.et");
		if (!lightPrefab || !lightPrefab.IsValid()) return;
		m_StudioKey = LightEntity.Cast(GetGame().SpawnEntityPrefab(lightPrefab, m_World));
		m_StudioFillLeft = LightEntity.Cast(GetGame().SpawnEntityPrefab(lightPrefab, m_World));
		m_StudioFillRight = LightEntity.Cast(GetGame().SpawnEntityPrefab(lightPrefab, m_World));
		if (m_StudioFillLeft) m_StudioFillLeft.SetRadius(4);
		if (m_StudioFillRight) m_StudioFillRight.SetRadius(4);
	}
	...
	protected void PollInput(float tDelta)
	{
		...
		if (m_InputManager.IsUsingMouseAndKeyboard())
		{
			...
			bool dragHeld = m_bPointerRotate || m_InputManager.GetActionValue("BIA_ArmoryDrag") > 0;
			int mouseDeltaX;
			int mouseDeltaY;
			if (dragHeld)
			{
			...
			bool panHeld = m_InputManager.GetActionValue("BIA_ArmoryPan") > 0;
			...
			if (m_bMousePan)
			{
				...
				Pan(m_InputManager.GetActionValue("BIA_ArmoryYaw"), m_InputManager.GetActionValue("BIA_ArmoryPitch"), Math.Max(m_fDist, 0.8) * 0.004);
			}
			else if (m_bMouseRotate)
			{
				if (m_bTranslationTrack)
				{
					float trackX = m_InputManager.GetActionValue("BIA_ArmoryYaw");
					float trackY = m_InputManager.GetActionValue("BIA_ArmoryPitch");
					if (trackX == 0)
						trackX = mouseDeltaX;
					if (trackY == 0)
						trackY = mouseDeltaY;
				...
				else
				{
					...
					float yawInput = m_InputManager.GetActionValue("BIA_ArmoryYaw");
			...
			if (IsOverStage(mouseX, mouseY)
				&& !BIA_SmoothScrollComponent.IsCursorOverAny(mouseX, mouseY)
				&& !BIA_CarouselComponent.IsCursorOverAny(mouseX, mouseY))
			{
				float wheel = m_InputManager.GetActionValue("BIA_ArmoryZoomWheel");
				if (wheel == 0)
		...
		else
		{
			...
			bool panPressed = m_InputManager.GetActionValue("BIA_ArmoryPanMode") > 0;
			if (panPressed && !m_bPanModeHeld)
				m_bPanArmed = !m_bPanArmed;
			m_bPanModeHeld = panPressed;

			bool resetPressed = m_InputManager.GetActionValue("BIA_ArmoryReset") > 0;
			...
			{
				Pan(-m_InputManager.GetActionValue("BIA_ArmoryYaw"), -m_InputManager.GetActionValue("BIA_ArmoryPitch"), Math.Max(m_fDist, 0.8) * 2 * tDelta);
			}
			else
			{
				float yawDelta = m_InputManager.GetActionValue("BIA_ArmoryYaw") * scale;
				if (m_bSubjectSpinYaw)
					m_OnSubjectSpin.Invoke(yawDelta);
				else
					m_vAnglesTarget[0] = ClampYaw(m_vAnglesTarget[0] + yawDelta);
			...
			float zoomDelta = m_InputManager.GetActionValue("BIA_ArmoryZoomIn") - m_InputManager.GetActionValue("BIA_ArmoryZoomOut");
			if (zoomDelta != 0)
	...
	protected void EnsureDust()
	{
		...
		if (!m_Dust)
			BIA_Log.Warn("StageCore: dust effect failed to spawn, stage stays clean");
	}
