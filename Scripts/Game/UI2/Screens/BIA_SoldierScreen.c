...
class BIA_SoldierCard
{
	string m_sLabel;
	BIA_ArmoryCategory m_Category;
	int m_iWeaponSlot = -1;
	int m_iClothingSlot = -1;
	string m_sArea;
	BIA_ItemRowComponent m_Row;
...
enum BIA_ESoldierActionKind
{
	CONTENTS,
	ATTACHMENTS
}

class BIA_SoldierAction
{
	BIA_ESoldierActionKind m_eKind;
	string m_sLabel;
	BIA_ItemRowComponent m_Row;
}
...
class BIA_SoldierScreen : SCR_SubMenuBase
{
	...
	protected ref BIA_ItemListPanel m_ItemList;
	protected SCR_InputButtonComponent m_WearChip;
	protected ref array<ref BIA_SoldierCard> m_aCards = {};
	protected ref array<ref BIA_SoldierAction> m_aActions = {};
	protected BIA_SoldierCard m_ActiveCard;
	protected BIA_SoldierCard m_FocusedCard;
	protected BIA_SoldierAction m_ActiveAction;
	protected BIA_SoldierCard m_PendingFocusCard;
	protected ref array<ref BIA_ItemEntry> m_aActiveItems;
	protected bool m_bSuppressFocusFrame;

	protected static BIA_SoldierScreen s_ActiveInstance;
	...
	override void OnTabCreate(Widget menuRoot, ResourceName buttonsLayout, int index)
	{
		...
		m_ItemList = new BIA_ItemListPanel(m_wRoot, "ItemListPanel", "ItemListTitle", "SoldierItemList", "SoldierItemScroll", "SoldierSearchBox", "ItemListBackControls", "ItemListFilters");
	...
	{
		BIA_SoldierScreen screen = s_ActiveInstance;
		if (!screen || !screen.m_ItemList || !screen.m_ItemList.IsOpen())
			return false;

		if (screen.m_ActiveAction && screen.m_ActiveAction.m_eKind == BIA_ESoldierActionKind.CONTENTS)
			screen.ExitContents();
		else
	...
	protected void BuildCards()
	{
		...
		array<BIA_ArmoryCategory> outfitCategories = {};
		service.m_Config.GetCategoriesForTab(BIA_EArmoryTab.OUTFIT, outfitCategories);
		BIA_CatalogService.BuildCharacterSlotCategories(outfitCategories, character, service.m_Arsenal, service.GetBrowseFaction());
		...
		foreach (BIA_ArmoryCategory outfitCategory : outfitCategories)
		{
	...
	//------------------------------------------------------------------------------------------------
	protected void SpawnCard(notnull BIA_SoldierCard card, Widget listParent)
	{
		if (!listParent)
			return;
	...
	protected void RefreshCards()
	{
		...
		foreach (BIA_SoldierCard card : m_aCards)
		{
			if (!card.m_Row)
				continue;
			...
			string state = "EMPTY";
			if (!current.IsEmpty())
				state = BIA_CatalogService.GetDisplayName(current, faction);

			card.m_Row.SetSlotDisplay(label, state, current);
			bool hasContext = !current.IsEmpty()
				&& (BIA_ItemIntel.HasContainerStorage(current) || BIA_ItemIntel.HasVisibleAttachmentSlots(current));
			card.m_Row.SetContextHintVisible(hasContext);
		}
	}

	//------------------------------------------------------------------------------------------------
	protected ResourceName CurrentPrefabFor(notnull BIA_SoldierCard card, notnull BIA_DraftService service)
	{
		if (card.IsWeapon())
		{
			BIA_KitWeapon weapon = service.m_Draft.FindWeapon(card.m_iWeaponSlot);
			if (weapon)
				return weapon.m_Prefab;
			return ResourceName.Empty;
		}

		BIA_KitClothing clothing = service.m_Draft.FindClothing(card.m_iClothingSlot);
		if (clothing)
	...
	//------------------------------------------------------------------------------------------------
	protected BIA_SoldierCard CardForRow(BIA_ItemRowComponent row)
	{
		foreach (BIA_SoldierCard card : m_aCards)
		{
			if (card.m_Row == row)
	...
	//------------------------------------------------------------------------------------------------
	protected BIA_SoldierAction ActionForRow(BIA_ItemRowComponent row)
	{
		foreach (BIA_SoldierAction action : m_aActions)
		{
			if (action.m_Row == row)
				return action;
		}
	...
	//------------------------------------------------------------------------------------------------
	protected void RebuildActions(BIA_SoldierCard card)
	{
		ClearActions();
		...
		if (BIA_ItemIntel.HasVisibleAttachmentSlots(clothing.m_Prefab))
		{
			BIA_SoldierAction attachments = new BIA_SoldierAction();
			attachments.m_eKind = BIA_ESoldierActionKind.ATTACHMENTS;
	...
	//------------------------------------------------------------------------------------------------
	protected void SpawnAction(notnull BIA_SoldierAction action, string state, ResourceName thumbnail)
	{
	...
	//------------------------------------------------------------------------------------------------
	protected void OnCardFocused(BIA_ItemRowComponent row)
	{
		BIA_SoldierCard card = CardForRow(row);
	...
	protected void OnCardSecondaryClicked(BIA_ItemRowComponent row)
	{
		BIA_SoldierCard card = CardForRow(row);
	...
	//------------------------------------------------------------------------------------------------
	protected void OnCardClicked(BIA_ItemRowComponent row)
	{
		BIA_SoldierCard card = CardForRow(row);
		if (!card || !m_ItemList)
			return;
		...
		m_ActiveCard = card;
		m_ActiveAction = null;
		m_aActiveItems = BIA_CatalogService.GetItems(card.m_Category, service.GetBrowseFaction(), service.m_Arsenal, service.GetCostType());
		if (!m_aActiveItems)
			return;
	...
	//------------------------------------------------------------------------------------------------
	protected void OnActionClicked(BIA_ItemRowComponent row)
	{
		BIA_SoldierAction action = ActionForRow(row);
	...
	//------------------------------------------------------------------------------------------------
	protected void OpenAction(notnull BIA_SoldierCard card, notnull BIA_SoldierAction action)
	{
		if (!m_ItemList)
			return;
		...
		if (action.m_eKind == BIA_ESoldierActionKind.ATTACHMENTS)
		{
			if (!BIA_ShellMenu.OpenGunsmithForClothing(card.m_iClothingSlot))
				SCR_UISoundEntity.SoundEvent(SCR_SoundEvent.CLICK_FAIL);
			return;
		}

		if (!m_aActiveItems)
			m_aActiveItems = new array<ref BIA_ItemEntry>();
		m_aActiveItems.Clear();

		if (action.m_eKind == BIA_ESoldierActionKind.CONTENTS)
		{
			BIA_CatalogService.CollectContentsItems(service, m_aActiveItems);
	...
	protected void CloseItemList(bool restoreFocus = true)
	{
		...
		BIA_SoldierCard closedCard = m_ActiveCard;
		BIA_SoldierAction closedAction = m_ActiveAction;
		m_ActiveCard = null;
		m_ActiveAction = null;
		m_aActiveItems = null;
	...
	{
		BIA_SoldierCard origin = m_FocusedCard;
		CloseItemList(false);
		ClearActions();
	...
	{
		BIA_SoldierCard card = m_PendingFocusCard;
		m_PendingFocusCard = null;
	...
	protected void OnListRowClicked(BIA_ItemRowComponent row)
	{
		if (!row || !row.GetEntry())
			return;
		...
		if (m_ActiveAction)
		{
			if (m_ActiveAction.m_eKind == BIA_ESoldierActionKind.CONTENTS)
	...
	//------------------------------------------------------------------------------------------------
	protected void OnListQtyDelta(BIA_ItemRowComponent row, int delta)
	{
		if (m_ActiveAction && m_ActiveAction.m_eKind == BIA_ESoldierActionKind.CONTENTS)
			ChangeContents(row, delta);
	}

	//------------------------------------------------------------------------------------------------
	protected void ChangeContents(BIA_ItemRowComponent row, int delta)
	{
		...
		BIA_KitClothing clothing = service.m_Draft.FindClothing(m_FocusedCard.m_iClothingSlot);
		if (!clothing || clothing.m_Prefab.IsEmpty())
			return;

		BIA_EExtraChangeResult result = service.ChangeDraftExtraTo(row.GetEntry().m_Prefab, delta, clothing.m_Prefab);
		...
		if (result != BIA_EExtraChangeResult.REMOVED)
		{
			if (m_ItemList)
	...
	//------------------------------------------------------------------------------------------------
	protected string ContentsTitle(notnull BIA_DraftService service, ResourceName container, string notice = "")
	{
	...
	//------------------------------------------------------------------------------------------------
	protected string ContainerUsageState(notnull BIA_DraftService service, ResourceName container)
	{
		float usedWeight;
		float usedVolume;
	...
	protected string ExtraRejectLabel(BIA_EExtraChangeResult result)
	{
		switch (result)
		{
			...
			case BIA_EExtraChangeResult.INCOMPATIBLE:
				return "DOES NOT FIT";
			case BIA_EExtraChangeResult.EMPTY:
				return "EMPTY";
		}
	...
	//------------------------------------------------------------------------------------------------
	protected void RefreshContentsCounts(notnull BIA_DraftService service, ResourceName container)
	{
