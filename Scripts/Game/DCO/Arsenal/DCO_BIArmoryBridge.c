...
// executed on the server by BIA_ResourcePlayerControllerInventory.
class DCO_BIArmoryBridge
{
	static bool IsOpen()
	{
		...
		if (!manager)
			return false;
		return manager.FindMenuByPreset(ChimeraMenuPreset.BIA_ArmoryV2) != null;
	}
	...
	protected static bool OpenForTarget(IEntity target, DCO_ArsenalAccessComponent access)
	{
		...
		if (access)
			BIA_Theme.BeginStationSession(access.GetAccentColor(), access.GetPanelOpacity());
		else
