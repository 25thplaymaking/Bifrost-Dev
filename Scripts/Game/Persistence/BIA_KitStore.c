...
class BIA_KitStore
{
	...
	//------------------------------------------------------------------------------------------------
	static bool SaveKit(notnull BIA_KitFile kit, int slot)
	{
		if (slot < 0)
			return false;
		...
		{
			BIA_Log.Warn(string.Format("KitStore: slot %1 export is empty or too large", slot));
			return false;
		}
	...
	//------------------------------------------------------------------------------------------------
	static BIA_KitFile LoadKit(int slot)
	{
		string json;
		if (UseConsolePath())
		{
		...
		{
			BIA_Log.Warn(string.Format("Kit slot %1 failed to parse", slot));
			return null;
		}
	...
	//------------------------------------------------------------------------------------------------
	static void GetKits(int slotCount, notnull out array<ref BIA_KitFile> kits)
	{
	...
	protected static void EnsureMigrated(int slotCount)
	{
		...
		if (migrated > 0)
			BIA_Log.Info(string.Format("Migrated %1 kit(s) into the shared bank", migrated));
	}
	...
	static int ImportLegacyKits(string currentFactionKey, int slotCount)
	{
		...
		array<ref BIA_KitFile> existing = {};
	...
	//------------------------------------------------------------------------------------------------
	protected static bool ImportOneKit(notnull BIA_KitFile kit, string currentFactionKey, int slotCount, notnull array<ref BIA_KitFile> existing)
	{
	...
	//------------------------------------------------------------------------------------------------
	protected static bool AlreadyImported(notnull array<ref BIA_KitFile> existing, notnull BIA_KitFile kit)
	{
		foreach (BIA_KitFile slotKit : existing)
		{
	...
	//------------------------------------------------------------------------------------------------
	protected static int FindFreeSlot(notnull array<ref BIA_KitFile> existing, int slotCount)
	{
		for (int slot = 0; slot < slotCount; ++slot)
		{
	...
	protected static GRSA_ArmorySettings ConsoleAcquire(out BaseContainer container)
	{
		...
		if (!container)
		{
			BIA_Log.Warn("BIA_ArmorySettings module not registered in user settings");
			return null;
		}
