...
class BIA_ClientPrefs
{
	...
	protected static ref BIA_ClientPrefs s_Instance;

	//------------------------------------------------------------------------------------------------
	void BIA_ClientPrefs()
	{
		SetDefaults();
	}
	...
	//------------------------------------------------------------------------------------------------
	static BIA_ClientPrefs Get()
	{
		if (!s_Instance)
		{
			s_Instance = new BIA_ClientPrefs();
	...
	static void Save()
	{
		BIA_ClientPrefs prefs = Get();
		if (BIA_KitStore.UseConsolePath())
		{
			prefs.WriteConsole();
			return;
		}
