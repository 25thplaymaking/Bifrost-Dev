class BIA_ItemRowQuantityHandler : ScriptedWidgetEventHandler
{
	protected BIA_ItemRowComponent m_Owner;
	protected int m_iDelta;

	void BIA_ItemRowQuantityHandler(BIA_ItemRowComponent owner, int delta = 0)
	{
		m_Owner = owner;
		m_iDelta = delta;
	}
...
class BIA_ItemRowComponent : SCR_ButtonBaseComponent
{
	protected ref BIA_ItemEntry m_Entry;
	...
	protected ref BIA_ItemRowQuantityHandler m_QtyEntryHandler;
	protected ref BIA_ItemRowQuantityHandler m_MinusHandler;
	protected ref BIA_ItemRowQuantityHandler m_PlusHandler;
	...
	protected bool m_bActivateOnPress;
	...
	//! (BIA_ItemRowComponent row, int delta)
	ref ScriptInvoker m_OnQtyDelta = new ScriptInvoker();
	//! (BIA_ItemRowComponent row, int value)
	ref ScriptInvoker m_OnQtySet = new ScriptInvoker();

	//------------------------------------------------------------------------------------------------
	override void HandlerAttached(Widget w)
	{
		m_BackgroundDefault = BIA_Theme.MapButtonAccent(m_BackgroundDefault);
		m_BackgroundHovered = BIA_Theme.MapButtonAccent(m_BackgroundHovered);
		m_BackgroundSelected = BIA_Theme.MapButtonAccent(m_BackgroundSelected);
		m_BackgroundSelectedHovered = BIA_Theme.MapButtonAccent(m_BackgroundSelectedHovered);
		m_BackgroundClicked = BIA_Theme.MapButtonAccent(m_BackgroundClicked);
		super.HandlerAttached(w);
		m_OnClicked.Insert(OnRowActivated);
		BIA_Theme.ApplyAccentWidgets(w);
		...
		if (m_wState)
			m_wState.SetColor(BIA_Theme.Accent());
		...
		if (m_wQtyEntry)
		{
			m_QtyEntryHandler = new BIA_ItemRowQuantityHandler(this);
			m_wQtyEntry.AddHandler(m_QtyEntryHandler);
		}
		m_wMinusButton = w.FindAnyWidget("QtyMinus");
		if (m_wMinusButton)
		{
			m_MinusHandler = new BIA_ItemRowQuantityHandler(this, -1);
			m_wMinusButton.AddHandler(m_MinusHandler);
		}
		m_wPlusButton = w.FindAnyWidget("QtyPlus");
		if (m_wPlusButton)
		{
			m_PlusHandler = new BIA_ItemRowQuantityHandler(this, 1);
	...
	//------------------------------------------------------------------------------------------------
	void SetEntry(BIA_ItemEntry entry, bool usesSupplies)
	{
		m_Entry = entry;
	...
	void SetThumbnail(ResourceName prefab)
	{
		...
		BIA_ItemIntel.SetThumbnail(m_wThumb, prefab);
	}
	...
	void SetIconTexture(ResourceName texture)
	{
		if (m_wThumb)
		{
			ItemPreviewManagerEntity previewManager = BIA_ItemIntel.GetPreviewManager();
			if (previewManager)
	...
	//------------------------------------------------------------------------------------------------
	void SetActivateOnPress(bool enabled)
	{
		m_bActivateOnPress = enabled;
	}
	...
	{
		if (button == 0 && m_bActivateOnPress)
			return true;
		if (button == 1 && m_bSecondaryClickEnabled)
			return true;
