...
class BIA_SmoothScrollComponent : ScriptedWidgetComponent
{
	...
	protected static ref array<BIA_SmoothScrollComponent> s_aInstances;

	protected static void EnsureInstances()
	{
		if (!s_aInstances)
			s_aInstances = new array<BIA_SmoothScrollComponent>();
	}
	...
	{
		EnsureInstances();
		foreach (BIA_SmoothScrollComponent instance : s_aInstances)
		{
