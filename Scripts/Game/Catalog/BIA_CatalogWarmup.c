...
//! dedicated servers — the server apply path (BIA_ApplyGate) reads catalogs only, and the
//! entity-derived intel feeds menus a dedicated server never draws.
class BIA_CatalogWarmup
{
	...
	protected ref array<ref BIA_WarmupItem> m_aPending = {};
	...
	protected void CollectItems()
	{
		...
		if (!catalogManager)
		{
			BIA_Log.Warn("Catalog warmup: no entity catalog manager, armory falls back to lazy caching");
			return;
		}
		...
		foreach (SCR_ArsenalItem arsenalItem : arsenalItems)
		{
			...
			BIA_WarmupItem item = new BIA_WarmupItem();
	...
	protected void Slice()
	{
		...
		while (budget > 0 && !m_aPending.IsEmpty())
		{
			...
			BIA_WarmupItem item = m_aPending[last];
			m_aPending.Remove(last);
			budget--;

			BIA_ItemIntel.GetClothAreaType(item.m_Prefab);
			BIA_ItemIntel.GetWeight(item.m_Prefab);
			BIA_CatalogService.GetDisplayName(item.m_Prefab, null);

			if (item.m_eType & SCR_EArsenalItemType.WEAPON_ATTACHMENT)
				BIA_ItemIntel.GetAttachmentClass(item.m_Prefab);

			if (item.m_eMode & (SCR_EArsenalItemMode.WEAPON | SCR_EArsenalItemMode.WEAPON_VARIANTS))
			{
				array<ResourceName> defaults = {};
				BIA_ItemIntel.GetDefaultAttachments(item.m_Prefab, defaults);
			}
