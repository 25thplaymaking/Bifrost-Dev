...
class BIA_KitFile
{
	...
	ref array<ref BIA_KitExtra> m_aExtras = {};
	...
	void CollectItemPrefabs(notnull out array<ResourceName> outPrefabs)
	{
		...
		foreach (BIA_KitExtra extra : m_aExtras)
		{
	...
	//------------------------------------------------------------------------------------------------
	protected void ReadNodeAttachments(notnull JsonLoadContext ctx, notnull array<ResourceName> outAttachments, array<int> outSlots = null, int depth = 0)
	{
		if (depth > 4) { BIA_LoadoutBridge.SkipStoragesArray(ctx); return; }
		...
		int firstPart = outAttachments.Count();
		for (int s = 0; s < storageCount; s++)
		{
			if (!ctx.StartObject())
				continue;
			...
			id.Split(":", idParts, false);
			typename storageType;
			if (!idParts.IsEmpty()) storageType = idParts[0].ToType();
			int priority = 3;
			if (storageType)
			{
				if (storageType.IsInherited(WeaponAttachmentsStorageComponent)) priority = 0;
				else if (storageType.IsInherited(ClothNodeStorageComponent)) priority = 1;
				else if (storageType.IsInherited(BaseEquipmentStorageComponent)) priority = 2;
			}
			// Match the live mount lookup; secondary stores have independent slot IDs.
			bool attachmentStorage = priority < selectedPriority;
			if (attachmentStorage)
			{
				selectedPriority = priority;
				while (outAttachments.Count() > firstPart)
				{
					outAttachments.Remove(outAttachments.Count() - 1);
					if (outSlots) outSlots.Remove(outSlots.Count() - 1);
				}
			}
			...
			if (ctx.StartMap("slots", slotsCount))
			{
				for (int i = 0; i < slotsCount; i++)
				{
					...
					if (attachmentStorage && !prefab.IsEmpty())
					{
						...
						if (outSlots)
						{
							if (depth == 0) outSlots.Insert(idxStr.ToInt());
							else outSlots.Insert(-1);
						}
					}

					if (attachmentStorage) ReadNodeAttachments(ctx, outAttachments, outSlots, depth + 1);
					else BIA_LoadoutBridge.SkipStoragesArray(ctx);
					ctx.EndObject();
				}
				ctx.EndMap();
			}
	...
	protected void SerializeSlotData(notnull JsonSaveContext ctx)
	{
		...
		for (int i = 0; i < m_aCaptureKeys.Count(); i++)
		{
			...
			ctx.StartObject();
			BIA_LoadoutBridge.ReadEntity(entity, ctx);
			ctx.EndObject();
		}
		for (int i = 0; i < m_aOverrideKeys.Count(); i++)
		{
	...
	protected bool Deserialize(notnull JsonLoadContext ctx)
	{
		...
		if (ctx.StartMap("slotData", slotCount))
		{
			for (int i = 0; i < slotCount; i++)
			{
				...
				BIA_LoadoutBridge.SkipStoragesArray(ctx);
				ctx.EndObject();
			}
			ctx.EndMap();
		}
		...
		if (ctx.StartArray("extras", extraCount))
		{
			for (int i = 0; i < extraCount; ++i)
			{
				...
				BIA_KitExtra extra = new BIA_KitExtra();
