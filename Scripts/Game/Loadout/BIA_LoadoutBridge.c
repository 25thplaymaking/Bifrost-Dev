...
class BIA_LoadoutBridge
{
	...
	//------------------------------------------------------------------------------------------------
	static bool ApplyStoragesOnto(IEntity entity, LoadContext context, InventoryStorageManagerComponent manager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
		if (!entity || !manager)
	...
	//------------------------------------------------------------------------------------------------
	protected static bool ApplyStoragesArray(IEntity owner, LoadContext context, InventoryStorageManagerComponent manager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
		int storageCount = 0;
	...
	//------------------------------------------------------------------------------------------------
	protected static void ApplySlotsMap(BaseInventoryStorageComponent storage, LoadContext context, InventoryStorageManagerComponent manager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
		int slotCount = 0;
	...
	static void ApplyNode(IEntity owner, LoadContext context, InventoryStorageManagerComponent manager, BaseInventoryStorageComponent parentStorage, int slotId, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
	...
	static void ApplyAttachmentsArray(IEntity owner, LoadContext context, InventoryStorageManagerComponent manager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
		...
		BaseInventoryStorageComponent topStorage = BIA_ItemIntel.AttachmentStorage(owner);
		if (!topStorage)
			return;
		...
		// Pinned items must occupy their requested mount or be reported as skipped.
		if (!pinBySlot.IsEmpty())
		{
		...
		foreach (int requestedSlot, ResourceName requestedPrefab : pinBySlot)
		{
			...
			if (pendingPin >= 0)
			{
				...
				stats.RecordSkip(requestedPrefab);
			}
		}
