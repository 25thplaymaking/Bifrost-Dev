...
class BIA_Kit
{
	...
	ref array<ref BIA_KitClothing> m_aClothings = {};
	ref array<ref BIA_KitWeapon> m_aWeapons = {};
	...
	//------------------------------------------------------------------------------------------------
	BIA_KitClothing FindClothing(int slotIdx)
	{
		foreach (BIA_KitClothing clothing : m_aClothings)
		{
	...
	//------------------------------------------------------------------------------------------------
	BIA_KitWeapon FindWeapon(int slotIdx)
	{
		foreach (BIA_KitWeapon weapon : m_aWeapons)
		{
	...
	{
		BIA_KitClothing clothing = FindClothing(slotIdx);
		if (prefab.IsEmpty())
		{
			if (clothing)
	...
	{
		BIA_KitWeapon weapon = FindWeapon(slotIdx);
		if (prefab.IsEmpty())
		{
			if (weapon)
	...
	BIA_KitExtra FindExtra(ResourceName prefab, ResourceName container)
	{
	...
	{
		BIA_KitExtra extra = FindExtra(prefab, container);
		if (extra)
		{
			if (count <= 0)
		...
		BIA_KitExtra newExtra = new BIA_KitExtra();
	...
	bool ReadFrom(notnull JsonLoadContext ctx)
	{
		...
		if (ctx.StartArray("clothings", clothingCount))
		{
			for (int i = 0; i < clothingCount; ++i)
			{
				...
				BIA_KitClothing clothing = new BIA_KitClothing();
		...
		if (ctx.StartArray("weapons", weaponCount))
		{
			for (int i = 0; i < weaponCount; ++i)
			{
				...
				BIA_KitWeapon weapon = new BIA_KitWeapon();
