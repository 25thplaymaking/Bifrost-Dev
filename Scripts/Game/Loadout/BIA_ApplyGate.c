...
class BIA_ApplyGate
{
	...
	//------------------------------------------------------------------------------------------------
	static BIA_ApplyGate Build(SCR_ArsenalComponent arsenal, notnull GameEntity character, notnull BIA_ArmoryConfig config)
	{
		BIA_ApplyGate gate = new BIA_ApplyGate();
		...
		if (scope == BIA_ECatalogScope.ALL_FACTIONS && config.m_bRestrictToArsenalConfig)
		...
		if (scope == BIA_ECatalogScope.STATION_ARSENAL && arsenal)
		{
			...
			else
			{
				...
				BIA_Log.Warn("Apply gate: station inventory unavailable, using the player's faction catalog");
			}
		}
		else if (scope == BIA_ECatalogScope.PLAYER_FACTION || scope == BIA_ECatalogScope.STATION_ARSENAL)
			gate.m_bRestrictToPlayerCatalog = true;
	...
	//------------------------------------------------------------------------------------------------
	//! Returns BIA_ArmoryCodes.OK to allow, else the skip-reason code.
	string Verdict(ResourceName prefab)
	{
		if (prefab.IsEmpty())
			return BIA_ArmoryCodes.ITEM_UNRESOLVABLE;
	...
	protected string ComputeVerdict(ResourceName prefab)
	{
		if (m_bRestrictToArsenal && !m_AllowedPrefabs.Contains(prefab))
			return BIA_ArmoryCodes.ITEM_NOT_IN_ARSENAL;
		...
		if (m_bRestrictItemSets || m_bRankLocked)
			itemData = BIA_CatalogService.FindArsenalItemData(prefab, m_Faction);

		if (m_bRestrictItemSets && !BIA_ArsenalScenarioSettings.Get().AllowsArsenalItem(itemData))
			return BIA_ArmoryCodes.ITEM_NOT_IN_ARSENAL;

		if (m_bRankLocked)
		{
			if (itemData && itemData.GetRequiredRank() > m_iPlayerRank)
				return BIA_ArmoryCodes.ITEM_RANK_LOCKED;
		}

		return BIA_ArmoryCodes.OK;
	}
	...
	{
		return Verdict(prefab) == BIA_ArmoryCodes.OK;
	}
}
