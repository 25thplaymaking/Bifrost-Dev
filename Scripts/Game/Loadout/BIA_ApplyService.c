...
class BIA_ApplyService
{
	...
	static BIA_ApplyReport ApplyKitFile(notnull GameEntity character, notnull BIA_KitFile kit, notnull BIA_ApplyContext ctx)
	{
		...
		{
			report.m_aSkippedNames.Insert(BIA_CatalogService.GetDisplayName(sample, faction));
		}
		...
		if (report.m_iSkipped > 0)
			report.m_eStatus = BIA_EApplyStatus.PARTIAL;
		else
	...
	protected static bool PassesFactionPolicy(notnull BIA_KitFile kit, string playerFactionKey, notnull BIA_ApplyContext ctx)
	{
		if (!BIA_ArsenalScenarioSettings.Get().m_bRestrictKitFaction)
			return true;
		...
		if (kit.m_iFactionStampVersion < BIA_KitFile.FACTION_STAMP_PLAYER || kit.m_sFactionKey.IsEmpty())
			return true;
	...
	//------------------------------------------------------------------------------------------------
	protected static void ApplySlotData(notnull GameEntity character, string rawJson, notnull InventoryStorageManagerComponent storageManager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats, out bool sawNestedStorage)
	{
		...
		if (context.StartMap("slotData", slotCount))
		{
			for (int i = 0; i < slotCount; i++)
			{
				...
				if (ResolveTopLevelTarget(character, loadoutStorage, weaponStorage, slotKey, parentStorage, slotId))
				{
					...
					BIA_LoadoutBridge.ApplyNode(existing, context, storageManager, parentStorage, slotId, gate, stats);
				}
				else
				{
					ResourceName prefab;
					...
					BIA_LoadoutBridge.SkipStoragesArray(context);
				}
	...
	{
		int weaponIdx = BIA_KitConvert.WeaponIdxFromKey(slotKey);
		if (weaponIdx >= 0)
		{
	...
	protected static void StripSlotsNotInKit(EquipedLoadoutStorageComponent loadoutStorage, EquipedWeaponStorageComponent weaponStorage, notnull set<string> kitKeys, notnull InventoryStorageManagerComponent storageManager)
	{
		...
		if (weaponStorage)
		{
			...
			for (int i = 0; i < slotsCount; ++i)
			{
				string key = BIA_KitConvert.WeaponSlotKey(i);
	...
	protected static void ApplyExtras(notnull GameEntity character, notnull BIA_KitFile kit, notnull InventoryStorageManagerComponent storageManager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
		PruneExtras(character, kit, storageManager, stats);
		if (kit.m_aExtras.IsEmpty())
			return;

		BIA_Kit current = BIA_KitCapture.CaptureFromCharacter(character);
		map<string, int> pairCounts = new map<string, int>();
		map<ResourceName, int> totalCounts = new map<ResourceName, int>();
		foreach (BIA_KitExtra existing : current.m_aExtras)
		{
		...
		foreach (BIA_KitExtra extra : kit.m_aExtras)
		{
	...
	//------------------------------------------------------------------------------------------------
	protected static void PruneExtras(notnull GameEntity character, notnull BIA_KitFile kit, notnull InventoryStorageManagerComponent storageManager, notnull BIA_ApplyStats stats)
	{
		...
		BIA_KitCapture.CollectExtraEntities(character, items, prefabs, containers);
	...
	//------------------------------------------------------------------------------------------------
	protected static void SpawnExtraCount(notnull GameEntity character, notnull InventoryStorageManagerComponent storageManager, BIA_ApplyGate gate, notnull BIA_ApplyStats stats, notnull BIA_KitExtra extra, int spawnCount)
	{
	...
	protected static void FindContainerStorages(notnull GameEntity character, ResourceName containerPrefab, notnull array<BaseInventoryStorageComponent> outStorages)
	{
		...
		for (int i = 0; i < slotsCount; ++i)
		{
			...
			BIA_ItemIntel.CollectContainerStorages(worn, outStorages);
			return;
		}
	}

	//------------------------------------------------------------------------------------------------
	protected static void GrantDefaultMagazines(notnull GameEntity character, notnull InventoryStorageManagerComponent storageManager, int magazineCount, BIA_ApplyGate gate, notnull BIA_ApplyStats stats)
	{
		EquipedWeaponStorageComponent weaponStorage = EquipedWeaponStorageComponent.Cast(character.FindComponent(EquipedWeaponStorageComponent));
		if (!weaponStorage)
			return;
	...
	//------------------------------------------------------------------------------------------------
	protected static bool UsesSupplies(notnull BIA_ApplyContext ctx)
	{
		bool enabled = BIA_ArsenalScenarioSettings.Get().m_bUseSupplies || ctx.m_Config.m_bUseSupplies;
		return enabled && ctx.m_Arsenal && ctx.m_Arsenal.IsArsenalUsingSupplies();
	}

	//------------------------------------------------------------------------------------------------
	protected static void ComputeDeltaCost(notnull BIA_KitFile kit, notnull GameEntity character, SCR_Faction faction, SCR_EArsenalSupplyCostType costType, BIA_ApplyGate gate, out float addedCost, out float removedRefund)
	{
		addedCost = 0;
		removedRefund = 0;
		...
		BIA_Kit currentKit = BIA_KitCapture.CaptureFromCharacter(character);
		map<ResourceName, int> current = new map<ResourceName, int>();
		foreach (BIA_KitClothing clothing : currentKit.m_aClothings)
			Increment(current, clothing.m_Prefab, 1);
		foreach (BIA_KitWeapon weapon : currentKit.m_aWeapons)
		{
			Increment(current, weapon.m_Prefab, 1);
			foreach (ResourceName attachment : weapon.m_aAttachments)
				Increment(current, attachment, 1);
		}
		foreach (BIA_KitExtra extra : currentKit.m_aExtras)
		...
		foreach (ResourceName prefab, int wantedCount : wanted)
		{
			...
			SCR_ArsenalItem itemData = BIA_CatalogService.FindArsenalItemData(prefab, faction);
			if (itemData)
	...
	//------------------------------------------------------------------------------------------------
	protected static void ChargeSupplies(SCR_ResourceConsumer consumer, SCR_ResourceGenerator generator, float addedCost, float removedRefund, notnull BIA_ApplyContext ctx, notnull BIA_ApplyReport report)
	{
		if (!consumer)
			return;
