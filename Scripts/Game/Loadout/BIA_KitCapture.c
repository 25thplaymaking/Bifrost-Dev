class BIA_KitCapture
{
	//------------------------------------------------------------------------------------------------
	static BIA_Kit CaptureFromCharacter(notnull GameEntity character, SCR_EArsenalSupplyCostType costType = SCR_EArsenalSupplyCostType.DEFAULT)
	{
		BIA_Kit kit = new BIA_Kit();
	...
	//------------------------------------------------------------------------------------------------
	protected static void CaptureClothings(notnull GameEntity character, notnull BIA_Kit kit)
	{
	...
	//------------------------------------------------------------------------------------------------
	protected static void CaptureWeapons(notnull GameEntity character, notnull BIA_Kit kit)
	{
	...
	protected static void CaptureAttachments(notnull IEntity owner, notnull array<ResourceName> outAttachments, notnull array<int> outPins)
	{
		...
		if (!storage)
		{
			BIA_ItemIntel.CollectSubtreeAttachments(owner, outAttachments);
		...
		for (int slotIdx = 0; slotIdx < slotsCount; ++slotIdx)
		{
			...
			BIA_ItemIntel.CollectSubtreeAttachments(attachment, outAttachments);
	...
	protected static void CaptureExtras(notnull GameEntity character, notnull BIA_Kit kit)
	{
		...
		notnull array<ResourceName> outContainers)
	{
		...
		foreach (IEntity worn : wornEntities)
		{
			...
			BIA_ItemIntel.CollectContainerStorages(worn, cargoStorages);
		...
		foreach (IEntity item : items)
		{
			...
			if (parentSlot && BIA_ItemIntel.IsAttachmentStorage(parentSlot.GetStorage()))
				continue;
	...
	//------------------------------------------------------------------------------------------------
	static void BuildSummaries(notnull BIA_Kit kit, SCR_Faction faction)
	{
		string weaponsSummary;
		foreach (BIA_KitWeapon weapon : kit.m_aWeapons)
		{
			if (!weaponsSummary.IsEmpty())
				weaponsSummary += ", ";
			weaponsSummary += BIA_CatalogService.GetDisplayName(weapon.m_Prefab, faction);
		}
		kit.m_sSummaryWeapons = weaponsSummary;

		string clothingSummary;
		int clothingListed = 0;
		foreach (BIA_KitClothing clothing : kit.m_aClothings)
		{
			if (clothingListed >= 3)
				break;
			...
			if (!clothingSummary.IsEmpty())
				clothingSummary += ", ";
			clothingSummary += BIA_CatalogService.GetDisplayName(clothing.m_Prefab, faction);
			clothingListed++;
		}
	...
	//------------------------------------------------------------------------------------------------
	static void ComputeCost(notnull BIA_Kit kit, SCR_Faction faction, SCR_EArsenalSupplyCostType costType)
	{
		float cost = 0;
		int requiredRank = 0;
		...
		{
			SCR_ArsenalItem itemData = BIA_CatalogService.FindArsenalItemData(prefabName, faction);
			if (!itemData)
				continue;
		...
		{
			SCR_ArsenalItem itemData = BIA_CatalogService.FindArsenalItemData(extra.m_Prefab, faction);
			if (!itemData)
				continue;
