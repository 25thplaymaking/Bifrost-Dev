...
class BIA_KitConvert
{
	...
	static BIA_KitFile CaptureFromCharacter(notnull GameEntity character, string kitName)
	{
	...
	//------------------------------------------------------------------------------------------------
	protected static void CaptureEquipmentSlots(notnull GameEntity character, EquipedLoadoutStorageComponent loadoutStorage, notnull BIA_KitFile kit)
	{
	...
	static BIA_KitFile FromDraft(notnull BIA_Kit draft, notnull GameEntity character, string kitName)
	{
		BIA_KitFile kit = new BIA_KitFile();
		kit.m_sName = kitName;
		kit.m_sKit = "kits";
		...
		EquipedLoadoutStorageComponent loadoutStorage = EquipedLoadoutStorageComponent.Cast(character.FindComponent(EquipedLoadoutStorageComponent));
		foreach (BIA_KitClothing clothing : draft.m_aClothings)
		{
			if (!loadoutStorage)
				break;
		...
		foreach (BIA_KitWeapon weapon : draft.m_aWeapons)
		{
		...
		foreach (BIA_KitExtra extra : draft.m_aExtras)
		{
			BIA_KitExtra copy = new BIA_KitExtra();
	...
	static void FlattenToDraft(notnull BIA_KitFile kit, GameEntity character, notnull BIA_Kit outDraft)
	{
		...
		for (int i = 0; i < slotCount; ++i)
		{
			...
			if (weaponIdx >= 0)
			{
				...
				BIA_KitWeapon draftWeapon = outDraft.FindWeapon(weaponIdx);
				if (draftWeapon)
				{
					...
					if (!explicitAttachments)
						BIA_ItemIntel.GetDefaultAttachments(prefab, draftWeapon.m_aAttachments);
			...
			if (clothingSlot >= 0)
			{
				...
				BIA_KitClothing draftClothing = outDraft.FindClothing(clothingSlot);
				if (draftClothing)
				{
					bool explicitAttachments = kit.GetSlotAttachments(key, draftClothing.m_aAttachments, draftClothing.m_aAttachmentSlots);
					if (!explicitAttachments)
						BIA_ItemIntel.GetDefaultAttachments(prefab, draftClothing.m_aAttachments);
	...
	static void ComputeSidecars(notnull BIA_KitFile kit, SCR_Faction faction, SCR_EArsenalSupplyCostType costType)
	{
		float cost = 0;
		int requiredRank = 0;
