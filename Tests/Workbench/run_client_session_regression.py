"""Native item-list and nested inventory graph checks."""
import json
import socket
import struct
...
def main():
    request = json.dumps({"APIFunc": "DCO_ClientSessionRegression"})
    ...
    if result.get("passed") != 21 or result.get("failures"):
        raise SystemExit(1)
