"""Run the field resource probe in an existing local Workbench play world."""
import json
import socket
import struct
...
def main():
    request = json.dumps({"APIFunc": "DCO_FieldFeatureRegression"})
    ...
    if result.get("passed") != 25 or result.get("failures"):
        raise SystemExit(1)
