...
- Fixed the mission editor's null-reference crash when marker snapshots are replaced. Preserved destination selection by ID, required a new choice after deletion, ignored stale or duplicate server replies, and prevented accidental conversion between intel and teleporter interactions. Added 18 isolated engine regression checks and repeated the failing consecutive-save workflow successfully in local Game Master.
- Added a shared Mission Tools editor for replicated Hide Terrain Objects, Scale Object, Create/Edit Intel, Global Hint, Chatter, Make Invincible, Create LZ/RP/Target with reusable named positions, and paired Create Teleporter interactions. Reuses native journals, hints, editable waypoints and authoritative teleportation; see the mission-tools implementation guide for supported targets, limits and pending multiplayer acceptance. Damage Buildings remains deferred because native collapse has no verified restoration path.
- Completed an exhaustive static review of Achilles at commit `f123656459cab7766aa40c32d5ee12d29ebadaae`, covering all nine addons, 294 SQF function files, 100 non-custom module/config entries, 50 user-defined slots, five scripted waypoints, ten CBA input outcomes, and 66 packaged Ares compositions. Added the safe missing outcome under its exact Achilles name: server-authoritative, natively replicated and JIP-restored `Change Side Relations`.
- Completed an exhaustive static review of ZEN at commit `b5786646a397593831232675a138cd6e35924356`, covering all 36 addons, 64 documented modules, 19 context-action groups, and 450 prepared functions. Added the safe missing outcomes under their exact ZEN names: native replicated `Suppressive Fire`, direct local/server `Place Comment`, and server-authoritative selected-set `Teleport Players`.
- Made compositions accessible from Game Master action menus. Selected objects expose `Save Selection as Composition`, which opens the persistent library in capture mode with the selection count and name field ready. Composition and mission setup fields have bordered text boxes, persistent labels and examples, input limits, and short instructions.
- Restored complete Loiter and Air Support gunrun presentation: LIVE shots now broadcast their tracer cue as well as spawning authoritative ammunition, COSMETIC shots retain tracer and matching weapon audio, and Game Masters can select among six vanilla tracer calibers whose ballistics and sound are snapshotted per pass.
- Migrated every persistent Game Master action cue from legacy debug shapes to Bifrost's client canvas. Triggers now show their replicated rotation, ellipse/rectangle footprint, finite height, and runtime color on remote and reconnecting Game Masters; task zones, QRF range previews, tactics placement previews, tracer aim/sound ranges, strike and mortar ranges, and CQB building bounds use the same visual path.
