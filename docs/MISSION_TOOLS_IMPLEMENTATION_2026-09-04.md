...
| A1 | Scale Object | Absolute uniform scale, 0.25–4, on selected static editable props, structures and whole barricade assemblies. Validates the physical hierarchy, including model-less composition roots, up to 512 parts. Excludes characters, vehicles, inventory items, systems, dynamic/kinematic parts and selection of an attached part alone, with explicit rejection reasons. Replicated root scale and hierarchy transform refresh preserve authored child offsets/scales. Existing composition placement applies this state to eligible roots. |
...
3. Select a static prop and a whole barricade assembly. Apply 0.5, 2.0, both limits and then 1.0. Verify every part's size, relative position and collision; move the assembly afterward, reconnect and place a saved composition containing it. Compare transforms across machines. Confirm characters, vehicles and assemblies with moving parts are rejected with an explanation and without modification. In a mixed selection, verify resized/skipped counts.
