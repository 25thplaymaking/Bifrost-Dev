...
| A1 | Scale Object | Runtime uniform scale, 0.01–100, on any replicated GM-editable entity category, including characters, vehicles, equipment and assemblies. Player delegates resolve to their controlled character. No class, physics or hierarchy bans; a selected parent covers nested selections. The server checks native scalar readback and replicates the desired value; explicitly resized entities restore scale after native movement/animation updates, including on proxies/JIP. Scale 1 removes Bifrost's extra frame check. No static transform reset, simulation disabling or replacement entity. Native collision, animation, vehicle-seat and equipment behavior requires asset-specific runtime verification. |
...
3. Follow the [runtime scale acceptance steps](RUNTIME_SCALE_2026-09-04.md). Cover a barricade, loose equipment, a vehicle, AI and a controlled player character; verify 0.5, 2.0 and reset to 1.0, movement/animation, collision and equipment. Repeat on a dedicated server with a remote GM and after JIP. Save/place a scaled composition and compare transforms. Check single attached-part selection and parent-plus-child selection without double application. Confirm invalid values, stale targets and unauthorized clients cannot modify state.
